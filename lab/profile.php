<?php
// lab/profile.php - Edit Lab Technician Profile Settings
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE lab_techs SET phone = ? WHERE tech_id = ?");
        if ($stmt->execute([$phone, $tech_id])) {
...
// Fetch current data
try {
    $stmt = $pdo->prepare("SELECT * FROM lab_techs WHERE tech_id = ?");
    $stmt->execute([$tech_id]);
    $tech = $stmt->fetch();

    // Fetch Lab Inventory Alerts for sidebar badge
...
<head>
    ...
    <title>Profile Settings - LuminaCare Lab</title>
...
<body>
        ...
        <div class="nav-menu">
            ...
            <div class="nav-item">
                <a href="inventory.php" class="nav-link">
                    ...
                    <?php if (isset($lab_inventory_alerts) && $lab_inventory_alerts > 0): ?>
    ...
    <main class="main-content">
        <div class="d-flex align-items-center mb-4 gap-3">
            ...
            <h3 class="mb-0 fw-bold">Lab Tech Profile Settings</h3>
        </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4">
                    <form action="profile.php" method="POST">
                        <div class="d-flex align-items-center gap-3 mb-4 border-bottom pb-4">
                            ...
                            <div>
                                <h5 class="fw-bold mb-1">Tech. <?php echo htmlspecialchars($tech['first_name'] . ' ' . $tech['last_name']); ?></h5>
                                <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1 rounded-pill"><i class="bi bi-shield-check me-1"></i> Authorized Lab Personnel</span>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control bg-light border-0" value="<?php echo htmlspecialchars($tech['first_name']); ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control bg-light border-0" value="<?php echo htmlspecialchars($tech['last_name']); ?>" readonly>
                            </div>
                            <div class="col-12">
                                ...
                                <input type="email" class="form-control bg-light border-0" value="<?php echo htmlspecialchars($tech['email']); ?>" readonly>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label text-muted small fw-bold">Contact Phone Number</label>
                                <input type="text" name="phone" class="form-control bg-light border-0" value="<?php echo htmlspecialchars($tech['phone'] ?? ''); ?>" placeholder="e.g. +1 234 567 8900">
                            </div>
                        </div>
                        <button type="submit" name="update_profile" class="btn btn-warning rounded-pill mt-4 px-4 fw-bold" style="background-color: var(--primary-color); border: none; color: white;">Save Changes</button>
                    </form>
                </div>
            </div>
            <div class="col-xl-4">
                <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                    <h6 class="fw-bold mb-3 text-dark"><i class="bi bi-info-circle text-primary me-2"></i> Account Support</h6>
                    <p class="small text-muted mb-4">Core identity details, including your professional email and name, are verified and managed by LuminaCare administration to maintain medical integrity. If you need to update these fields, please contact your System Administrator.</p>
