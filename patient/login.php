<?php
// patient/login.php - Patient Login Page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT patient_id, first_name, last_name, password_hash FROM patients WHERE email = ?");
            $stmt->execute([$email]);
            $patient = $stmt->fetch();

            // Verify if patient exists and password is correct
            if ($patient && password_verify($password, $patient['password_hash'])) {
                // Prevent Session Fixation
                session_regenerate_id(true);

                // Set session variables directly (Bypassing MFA)
            ...
            // error_log($e->getMessage()); // For production logging
        }
    }
}
?>
...
<head>
    ...
    <title>Patient Login - LuminaCare</title>
        ...
        <div class="auth-container row g-0">
            ...
            <div class="col-md-5 auth-sidebar">
                <div>
                    ...
                    <p class="opacity-75 mt-3 fs-5">Log in to track your vitals, view doctor messages, and manage your maternal health journey.</p>
                </div>
                
                <div class="mt-5 pt-5 border-top border-light border-opacity-25 z-1">
                    <p class="mb-2">Don't have an account yet?</p>
                    <a href="register.php" class="btn btn-outline-light rounded-pill px-4 fw-bold">Register Now</a>
                </div>
            </div>
            ...
            <div class="col-md-7 auth-form-wrapper">
                <div class="mb-4 text-center text-md-start">
                    <h3 class="fw-bold text-dark">Patient Login</h3>
                ...
                <form method="POST" action="login.php">
                    ...
                    <button type="submit" class="btn btn-primary-custom fs-5 mt-2">Log In</button>
